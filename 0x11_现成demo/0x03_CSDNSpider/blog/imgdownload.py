...
def dealfile(filename):
    ...
    #替换所有的<br/>符号
    try:
        print("准备开始替换所有的符号<br/>------------------------------------")
        filecontent = filecontent.replace(f"<br/>","")
        print("所有的<br/>符号均已替换完毕------------------------------------")
    except Exception as e:
        raise(e)
    

    # 匹配所有的图片连接
    urls = re.findall(r"https://i-blog.csdnimg.cn/blog_migrate/[\w]+.png", filecontent)    
    ...
    for url in urls:
        try:
            ...
            filecontent = filecontent.replace(extracontent, "", 1)
            print("-----------------------------------------------------------")
        except:
            print("图像写入失败-----------------------------------------------")
            print("-----------------------------------------------------------")
