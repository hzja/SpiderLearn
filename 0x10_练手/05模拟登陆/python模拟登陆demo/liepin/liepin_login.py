...
class Leipin(object):
    ...
    def _md5(self):
        ...
        md5.update(str.encode(password))#制定需要加密的字符串
        psw = md5.hexdigest()#获取加密后的16进制字符串
        return psw
    ...
    def _getAuthcode(self):
        ...
        imageUrl = selector.select('div > img')[0]['src']#select函数,用于查找html中我们所需要的内容
        authcodeUrl = 'https://passport.liepin.com{}'.format(imageUrl)#format()函数用于将字符串格式化，即将imageUrl代入到{}里
