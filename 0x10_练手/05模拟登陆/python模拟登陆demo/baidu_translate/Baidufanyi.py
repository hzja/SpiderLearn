...
context = js2py.EvalJs()#函数用于执行js代码
...
class BaiDuTranslater(object):
    ...
    def make_sign(self):
        ...
        with open("translate.js", "r", encoding="utf-8") as f:
            context.execute(f.read())#执行js代码

        sign = context.a(self.query)# 调用js中的a函数生成sign
        # 将sign加入到data中
        return sign
    ...
    def make_data(self, sign):
        ...
        if translating_mode == 0:
            from_str = "en"#需要翻译的是英语
            to_str = "zh"#翻译成中文
        else:
            from_str = "zh"#需要翻译的是中文
            to_str = "en"#翻译成英文
        data = {
            "query": self.query,
            "from": from_str,
