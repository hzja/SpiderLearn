...
class Lagou_login(object):
    def __init__(self):
        ...
        """
        #os.path.realpath(__file__)用于获取当前文件的绝对路径;
        os.path.split()用于将路径和文件名分开;
    ...
    def encryptPwd(self, passwd):
        ...
        """
        passwd.encode()从url进行用户名密码验证，为避免密码中有特殊字符时导致参数乱码，先对参数进行URLEncoder.encode处理;
        hashlib.md5(),对于md5对象，md5不能反解，但是加密是固定的，就是关系是一一对应，所以有缺陷，可以被对撞出来
        hashlib.md5().hexdigest()拿到加密对象
        """
    ...
    def getTokenCode(self):
        ...
        返回string中与pattern匹配的所有未重叠的值，包括空匹配值。
        """
    ...
    def getCaptcha(self):
        ...
        f.close()
        # 打开验证码图片
        if sys.platform.find('darwin') >= 0:#sys.platform获取当前系统平台
            subprocess.call(['open', self.CaptchaImagePath])#用subprocess.call()函数运行shell命令
        elif sys.platform.find('linux') >= 0:
            subprocess.call(['xdg-open', self.CaptchaImagePath])
        else:
            os.startfile(self.CaptchaImagePath)#os.startfile()相当于双击对应位置的文件
    ...
    def login(self, user, passwd, captchaData=None, token_code=None):
        ...
        # 头信息添加tokena
        login_headers = self.HEADERS.copy()#copy()函数用于复制
        token_code = self.getTokenCode() if token_code is None else token_code
        login_headers.update(token_code)#update()函数用于将两个字典合并操作，有相同的就覆盖
