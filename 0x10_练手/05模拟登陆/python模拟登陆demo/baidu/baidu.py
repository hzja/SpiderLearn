...
if (sys.version_info < (3, 0)):
    input = raw_input#如果python的版本低于3.0，则令input等于raw_input
...
class BaiduLogin(object):
    def __init__(self):
        ...
        self.sess = requests.session()#保持会话，使在跨请求时保存某些参数
        self.gid = str(uuid4()).upper()#uuid:互联网全局唯一标识符；uuid4:随机值;upper()此方法返回只包含大写字符的副本
        self.token = None
        self.key = None
    ...
    def _init_cookies(self):
        ...
        :return:
        """
        self.sess.get(url='https://pan.baidu.com/', headers=self.headers)#创建session对象，通过预先创建会话对象，可以重用会话同时可重用连接池保持与同一主机的连接
