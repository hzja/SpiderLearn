...
![微信截图_20250619202231](./img/微信截图_20250619202231.png)



​	查看网页源代码，直接搜索 textarea，没有的，试试直接搜索数据也是没有：

![微信截图_20250619203509](./img/微信截图_20250619203509.png)

![微信截图_20250619203541](./img/微信截图_20250619203541.png)
...
![微信截图_20250619204616](./img/微信截图_20250619204616.png)



​	这里我们想到一个 JavaScript 语法，如需从 JavaScript 访问某个 HTML 元素，可以使用 `document.getElementById(id)` 方法，这个 id 就是某个 HTML 元素的属性，然后使用 `innerHTML` 来获取或插入元素内容即可。
...
![微信截图_20250619205429](./img/微信截图_20250619205429.png)
...
![微信截图_20250622012123](./img/微信截图_20250622012123.png)
 ...
 var u = function() {
  ...
  function main(){
    var O = ""
        ...
        , ga = RegExp.$1;
    if (O.length)
        null == O.match(/^(\d+m|\d+p|\d+s|[1234567]+z)*$/) ? document.getElementById("tehai").innerHTML = "<font color=#FF0000 >INVALID QUERY</font>" : O.length && fa(O);
    else {
        var P, Q = "", S = Math.floor(3 * Math.random()), Q = Q + "<table cellpadding=0 cellspacing=0 ><tr><td>";
        var EncrpytData = [];
        for (P = 9; 0 <= P; --P) {
            for (var T, ha = 3 < P ? 136 : 36, U = void 0, V = [], U = 0; U < ha; ++U)
                V.push(U);
            for (U = 0; U < V.length - 1; ++U) {
                var W = U + Math.floor(Math.random() * (V.length - U))
                , ia = V[U];
                V[U] = V[W];
                V[W] = ia
            }
            T = V;
            var X = T.splice(0, 9 == P ? 4 : 8 == P ? 7 : 7 == P ? 10 : 13).sort(function(b, a) {
                return b - a
            }), Y = T.splice(0, 1)[0], Z, O = "";
            for (Z = 0; Z < X.length; ++Z)
                O += H(X[Z] + (3 < P ? 0 : 36 * S));
            -1 != Y && (O += H(Y + (3 < P ? 0 : 36 * S)),
            X.push(Y));
            var ja = G(X, X.length)
                , Q = Q + ('■<a href="?q=' + J(O) + '" class=D >')
                , Q = Q + N(ja, 14 == X.length)
                , Q = Q + "<br>";
            EncrpytData.push(fa(J(O)));
            for (Z = 0; Z < O.length; Z += 2)
                Q += L(O.substr(Z, 2), 2 == Z % 6 && Z == O.length - 2 ? " hspace=3 " : "");
            Q += "</a>";
            Q += "<br><br>";
            4 == P && (O=J(O))
        }
